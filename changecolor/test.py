...
def button_click(button_num):
    if button_num == 1:
        # 运行snake.py文件
        subprocess.Popen(["python", "./snake/snake.py"])
    elif button_num == 2:
        # 运行record.py文件
        subprocess.Popen(["python", "./money/record.py"])
    elif button_num == 3:
        # 弹出确认窗口
        response = messagebox.askokcancel("确认", "请确保input.docx放入根目录")
        if response:
            # 运行test.py文件
            subprocess.Popen(["python", "./translate/test.py"])
    elif button_num == 4:
...
def create_change_background_window():
    ...
    def change_background_color(color):
        if color == "红底":
            subprocess.Popen(["python", "hongdi.py"])
        elif color == "白底":
            subprocess.Popen(["python", "baidi.py"])
        elif color == "蓝底":
            subprocess.Popen(["python", "landi.py"])
